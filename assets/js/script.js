...
function callApi () {
    ...
    $.get(url, function (apiData) {
        ...
        var relatedSearch = apiData.Search[0].Poster;
        ...
        for (var i = 0; i < apiData.Search[i].Poster.length && i < imdburl.length; i++) {
            document.getElementById('relatedsearch').innerHTML="<h1><a href=" +imdburl[i]+"><img id='relatedimg' src="+apiData.Search[i].Poster+"></a></h1>"
            if(relatedSearch[i] === 3) {
                break;
            }
        }
